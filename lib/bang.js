...
const BangServer = function (services, config) {
	...
	app.get('/search', function (req, res) {
		...
		requestedHelp ?     serveHelp(res) :     redirectBrowser(res,
		redirected)
	...
	app.use(function (err, _, _, _) {
		...
		logger.error(err.stack, utils.debugInfo())
	...
	process.on( 'SIGTERM', shutdown(uptime) )
	process.on( 'SIGINT',  shutdown(uptime) )
	...
	return app.listen(config.port, function () {
		logger.info('Bang! %s listening at http://localhost:%d (pid %d).', constants.version, config.port, process.pid)
	})
