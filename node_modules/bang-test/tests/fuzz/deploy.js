
"use strict"




var bangServer = require('../app/bang')
var engines    = require('../data/engines.json')
var fuzz       = require('bang-test/tests/fuzz/fuzz')





var deploy = {
	fuzz: { }
}





deploy.normal = (args, callback) => {

	var state = {errors: [ ]}

	if (args.server) {
		args.server.fatalErrorHandler = args.server.fatalErrorHandler.bind({ }, state.errors)
	}

	bangServer(args.server, (app, server) => {

		state.server = {app, server}
		callback(state)

	})

}





var setupFuzzTest = (fuzzMethod, args, callback) => {

	var state = {errors: [ ]}

	if (args.server) {
		args.server.fatalErrorHandler = args.server.fatalErrorHandler.bind({ }, state.errors)
	}

	bangServer(args.server, (app, server) => {

		state.server = {app, server}

		fuzz[fuzzMethod](args.fuzz, args.engines, results => {

			state.results = results
			callback(state)

		})

	})

}





Object.keys(fuzz).forEach(method => {
	deploy.fuzz[method] = setupFuzzTest.bind({ }, method)
})





module.exports = deploy
