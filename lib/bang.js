...
/*
	dropNonLocalRequests :: req -> undefined

	close requests not sent over the loopback device. This prevents
	other machines using bang.

*/

const dropNonLocalRequests = function (req, res) {

	const isLoopback = /^127\./

	if ( !isLoopback.test(getIP(req)) ) {

		res.status(403).send("bugger off")
		logger.security( from("someone from the ip %s tried to connect.", getIP(req)) )
...
const BangServer = function (services, config) {
	...
	app.get('/suggest', function (req, res) {
		...
		*/
		...
		//dropNonLocalRequests(req)

		const rawQuery    = req.query.q || ""

		const redirected  = rd.redirect(rawQuery)

		giveSuggestions(res, redirected)
	...
	.listen(config.port, function () {
		...
		logger.info(
			format('Bang! %s listening at http://localhost:%d (pid %d).',
				constants.version, config.port, process.pid))

	})
	.on('error', function (err) {

		if (err.errno === 'EADDRINUSE') {
			logger.error(format('port %s currently in use.', config.port))
		} else {
