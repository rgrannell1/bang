...

{
	...
	*/
	...
	Engine.precond = engine => {
		...
		is.always.array(engine.patterns)

		engine.patterns.forEach(is.always.string)

		is.always.string(engine.searchTemplate)

		if (!is.undefined(engine.baseURL)) {
			is.always.string(engine.baseURL)
		}

		is.always.string(engine.suggestTemplate)
