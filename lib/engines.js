...
/*
	Engine :: [string] x string x string -> {
		regexp:          regex,
		asQueryURL:      string -> string,
		asSuggestionURL: string -> string,
		patterns:        [string],
		hostname:        string
	}
	...
	the engine object includes closures for filling in the templates with
	search terms, a regular expression for checking if a raw query should
	be handled by this engine, and the hostname of the search engine.
...
const Engine = function (patterns, searchTemplate, suggestTemplate) {
	...
	*/
	...
	const engineRegexp = new RegExp(escapedPattern, 'i')
	...
	return {
		...
		extractSearchTerms: function (searchTerms) {
			return encodeURIComponent(searchTerms.replace(engineRegexp, ''))
		},
