...
fuzzTest.noPattern = (ticksLeft, callback) => {
	...
	var recur = ticksLeft => {
		...
		if (ticksLeft === 0) {
			...
			setTimeout(callback, 0, startingTicks, testResults)

		} else {

			setTimeout(( ) => {

				var url = random.bangURL.search(constants.TEST_PORT, constants.UPPER_PATH_LENGTH)

				commons.callServer(url, testResult => {
...
describe(`# server url handling (n = ${config.TESTS})`, function ( ) {
